...
from utils.utils_restoration import psnr, modcrop, rescale, array2tensor, tensor2array, get_parameters, create_out_dir, single2uint,crop_center, matlab_style_gauss2D, imread_uint, imsave
...
def SR():
    ...
    if hparams.kernel_path is not None : # if a specific kernel saved in hparams.kernel_path as np array is given 
        ...
        # Kernels follow the order given in the paper (Table 2). The kernels are motion blur kernels.
    ...
    for k_index in k_index_list : # For each kernel
        ...
        exp_out_path = create_out_dir("../../Result_ERED", hparams, k_index = k_index)
        ...
        for sf in sf_list :
            ...
            if PnP_module.hparams.opt_alg == 'RED_Prox' or PnP_module.hparams.opt_alg == 'RED' or PnP_module.hparams.opt_alg == 'ERED':
                PnP_module.lamb, PnP_module.sigma_denoiser, PnP_module.maxitr, PnP_module.thres_conv, PnP_module.stepsize, PnP_module.std_0, PnP_module.std_end, PnP_module.lamb_0, PnP_module.lamb_end, _  = get_parameters(
            ...
            if PnP_module.hparams.opt_alg == 'SNORE_Prox' or PnP_module.hparams.opt_alg == 'SNORE':
                PnP_module.lamb, PnP_module.sigma_denoiser, PnP_module.maxitr, PnP_module.thres_conv, PnP_module.stepsize, PnP_module.std_0, PnP_module.std_end, PnP_module.lamb_0, PnP_module.lamb_end, _ = get_parameters(
                                        hparams.noise_level_img, hparams, k_index=k_index, degradation_mode='SR')
                PnP_module.std = PnP_module.sigma_denoiser = PnP_module.std_end
                PnP_module.lamb = PnP_module.lamb_end
            ...
            for i in range(min(len(input_paths),hparams.n_images)) : # For each image
                ...
                if hparams.extract_images or hparams.extract_curves or hparams.print_each_step:
                    ...
                    deblur_im, init_im, output_psnr, output_ssim, output_lpips, output_brisque, _, _, _, _, _, _, n_it = PnP_module.restore(blur_im.copy(),init_im.copy(),input_im.copy(),k, extract_results=False, sf=sf)
