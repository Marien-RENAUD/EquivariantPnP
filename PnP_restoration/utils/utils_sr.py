...
def shift_pixel(x, sf, upper_left=True):
    ...
    h, w = x.shape[:2]
    shift = (sf - 1) * 0.5
    xv, yv = np.arange(0, w), np.arange(0, h)
    ...
    if x.ndim == 2:  # Single channel
        ...
        x_shifted = interpolator(np.array(np.meshgrid(y1, x1)).T.reshape(-1, 2)).reshape(h, w)
    
    elif x.ndim == 3:  # Multi-channel
        x_shifted = np.zeros_like(x)
        for i in range(x.shape[-1]):  # Interpolate each channel independently
            interpolator = RegularGridInterpolator((yv, xv), x[:, :, i], method='linear', bounds_error=False, fill_value=None)
            x_shifted[:, :, i] = interpolator(np.array(np.meshgrid(y1, x1)).T.reshape(-1, 2)).reshape(h, w)
