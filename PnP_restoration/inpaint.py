...
from utils.utils_restoration import rescale, single2uint,crop_center, create_out_dir, get_parameters, matlab_style_gauss2D, imread_uint, imsave, psnr, array2tensor, tensor2array
...
def inpaint():
    ...
    if hparams.noise_level_img == None:
        hparams.noise_level_img = 0
    else:
        hparams.noise_level_img 
    hparams.n_init = 100
    hparams.use_backtracking = False
    
    # PnP_restoration class
    PnP_module = PnP_restoration(hparams)
    if not(PnP_module.hparams.opt_alg == 'SNORE_Prox' or PnP_module.hparams.opt_alg == 'SNORE'):
    ...
    for i in range(min(len(input_paths), hparams.n_images)): # For each image
        ...
        if hparams.noise_level_img > 0:
            ...
            mask_im = input_im*mask + (0.5)*(1-mask) + noise
        else:
        ...
        if hparams.extract_curves:
            # Create curves
            PnP_module.update_curves(x_list, psnr_tab, ssim_tab, brisque_tab, lpips_tab, Dg_list, g_list, F_list, nabla_F_list, f_list, lamb_tab, std_tab, estimated_noise_list)
