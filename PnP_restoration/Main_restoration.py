...
class PnP_restoration():
    ...
    def initialize_cuda_denoiser(self):
        ...
        elif self.hparams.denoiser_type == "DnCNN":
            self.denoiser_model = DnCNN(in_channels=3, out_channels=3, depth=20, bias=True, nf=64, pretrained='download', device=self.device)
            self.denoiser_model = self.denoiser_model.double()
        elif self.hparams.denoiser_type == "DRUNet":
            self.denoiser_model = DRUNet(in_channels=3, out_channels=3, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', pretrained='../GS_denoising/ckpts/drunet_deepinv_color.pth', device=self.device)
            self.denoiser_model = self.denoiser_model.double()
        else:
    ...
    def denoise(self, x, sigma, weight=1.):
        if self.hparams.denoiser_type == "GSDenoiser":
            ...
            torch.set_grad_enabled(True)
            Dx = self.denoiser_model.forward(x) # by default sigma = 2 / 255 with the pretrained weights
            torch.set_grad_enabled(False)
            return Dx, torch.tensor([0]).to(self.device), x - Dx
        elif self.hparams.denoiser_type == "DRUNet":
            torch.set_grad_enabled(True)
            Dx = self.denoiser_model.forward(x, sigma) # trained with default sigma in [0, 20] / 255 with the pretrained weights
            torch.set_grad_enabled(False)
            return Dx, torch.tensor([0]).to(self.device), x - Dx
        else:
            ValueError('Denoiser not implemented')
    ...
    def restore(self, img, init_im, clean_img, degradation, extract_results=False, sf=1):
        ...
        '''
        self.sf = sf
        if self.hparams.opt_alg == "SNORE" or self.hparams.opt_alg == "SNORE_Prox" or self.hparams.opt_alg == "PnP_SGD" or self.hparams.opt_alg == "ERED":
            self.hparams.use_backtracking = False
            self.hparams.early_stopping = False

        if extract_results:
            y_list, z_list, x_list, Dg_list, psnr_tab, ssim_tab, brisque_tab, lpips_tab, g_list, f_list, Df_list, F_list, nabla_F_list, Psi_list, lamb_tab, std_tab, estimated_noise_list = [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []

        # initalize parameters
        if (self.hparams.opt_alg == "RED_Prox" or self.hparams.opt_alg == "ERED_Prox" or self.hparams.opt_alg == "ERED" or self.hparams.opt_alg == "RED" or self.hparams.opt_alg == "Data_GD"):
        ...
        if extract_results:  # extract np images and PSNR values
            ...
            if self.hparams.print_each_step:
                print('current x PSNR : ', current_x_psnr.cpu())
                print('min:', float(torch.min(x0).cpu()))
                print('max:', float(torch.max(x0).cpu()))
        ...
        for i in tqdm(range(self.maxitr)):
            ...
            if self.hparams.opt_alg == "ERED" or self.hparams.opt_alg == "ERED_Prox":
                if extract_results:
                    lamb_tab.append(self.lamb); std_tab.append(self.std)
                    x_old_array = tensor2array(x_old)
                    if self.hparams.grayscale:
                ...
                if self.hparams.transformation == "rotation":
                    transform, inverse_transform = random_transform_rotation()
                if self.hparams.transformation == "flip":
                    transform, inverse_transform = random_transform_flip()
                if self.hparams.transformation == "translation":
                    transform, inverse_transform = random_transform_translation(x_old.shape[2], x_old.shape[3])
                if self.hparams.transformation == "all_transformations":
                    indx_transformation = np.random.randint(5)
                    if indx_transformation == 0:
                        transform, inverse_transform = random_transform_subpixel_rotation(self.device)
                    if indx_transformation == 1:
                        transform, inverse_transform = random_transform_rotation()
                    if indx_transformation == 2:
                        transform, inverse_transform = random_transform_flip()
                    if indx_transformation == 3:
                        transform, inverse_transform = random_transform_translation(x_old.shape[2], x_old.shape[3])
                    if indx_transformation == 4:
                ...
                Dx,g,Dg = self.denoise(x_old_transform, self.std)
                reg_grad = inverse_transform(x_old, Dg)                
                # Gradient regularization step
                z = x_old - self.stepsize * self.lamb * reg_grad
                # Data-fidelity step
                if self.hparams.opt_alg == "ERED_Prox":
                    x = self.data_fidelity_prox_step(z, img_tensor, self.stepsize)
            ...
            if self.hparams.opt_alg == "SNORE" or self.hparams.opt_alg == "SNORE_Prox":
                ...
                if i >= self.maxitr - self.hparams.last_itr:
                    self.std = self.std_end
                    self.lamb = self.lamb_end
                if self.hparams.inpainting_init and self.hparams.degradation_mode == 'inpainting':
                    if i < self.hparams.n_init:
                        self.std = 50. /255.
                        self.lamb = self.lamb_0
                    else :
                        self.std = self.std_end
                        self.lamb = self.lamb_end
                if extract_results:
                ...
                for ite in range(self.hparams.num_noise):
                    ...
                    if self.hparams.rot == True:
                        x_old_noise, angle = rotate_image_tensor(x_old_noise, self.device, generator = generator)
                    _,g,Dg = self.denoise(x_old_noise, self.std)
                    g_mean += g
                    if self.hparams.rot == True:
                        Dg, _ = rotate_image_tensor(Dg, self.device, angle = 360 - angle)
                    Dg_mean += Dg
                ...
                # Calculate Objective
                f, F = self.calculate_F(x, img_tensor, g=g)
                nabla_F = torch.sqrt(torch.sum((self.data_fidelity_grad(x_old, img_tensor) + self.lamb * Dg)**2)).item()
                y = x # output image is the output of the denoising step
            ...
            if self.backtracking_check : # if the backtracking condition is satisfied
                ...
                if extract_results:
                    ...
                    current_z_psnr = psnr_torch(clean_img_torch, z)
                    ...
                    psnr_tab.append(float(current_z_psnr.cpu()))
    ...
    def update_curves(self, x_list, psnr_tab, ssim_tab, brisque_tab, lpips_tab, Dg_list, g_list, F_list, nabla_F_list, f_list, lamb_tab, std_tab, estimated_noise_list):
    ...
    def add_specific_args(parent_parser):
        ...
        parser.add_argument('--transformation', type=str, choices=['rotation', 'subpixel_rotation', 'flip', 'translation','all_transformations'], help='Specify random transformation for ERED algorithm.', default='rotation')
        parser.add_argument('--opt_alg', dest='opt_alg', choices=['SNORE', 'Data_GD', 'SNORE_Prox', 'RED_Prox', 'ARED_Prox', 'RED', 'PnP_SGD', 'ERED', 'ERED_Prox'], help='Specify optimization algorithm')
        return parser